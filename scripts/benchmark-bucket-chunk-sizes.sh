...
for e in 32 16 8 4 2; do
    ...
    for c in 1 2 4 8 16 32; do
        ...
        (
            flock 9
            sed -Ei "s/^#define (SWAP_CHUNK_BUCKETS) .*\$/#define \\1 $c/" enclave/bucket.c
            set_sort_params_unlocked "$a" "$e" "$b" "$s" "$ENCLAVE_OFFSET" "$(( e + ENCLAVE_OFFSET - 1 ))"
        ) 9<.

        output_filename="$BENCHMARK_DIR/$a-sgx2-enclaves$e-bucketsize$BUCKET_SIZE-chunked$(( c * BUCKET_SIZE ))-elemsize$b-size$s-threads$t.txt"
